...
const Banner = ({ banerModal, data }) => {
    ...
    return (
        <div className='relative'>
            ...
            <img src={banerModal ? data?.poster_path?.[0]?.path : randomMovies?.poster_path?.[0]?.path} alt="background" className={`w-full object-cover z-0  ${banerModal ? '' : 'h-[600px]'}`} />
            ...
            <div className='px-12 absolute top-0 left-0 bottom-0 right-0 bg-gradient-to-t from-[rgba(0,0,0,0.5)] to-transparent  text-white'>
                ...
                <div className="absolute top-[70%]">
                    <div className="flex flex-col">
                        <p className='w-[45%] text-white text-[20px] leading-5 mb-5 ellipsis3'>{data?.overview || randomMovies?.overview}</p>
