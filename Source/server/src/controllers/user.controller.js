...
const signin = async (req, res) => {
  try {
    ...
    const payload = { id: user.id, roles: user.roles, infor: {id: user.id, displayName: user.displayName, username: user.username} };
    const token = jsonwebtoken.sign(payload, process.env.TOKEN_SECRET, { expiresIn: "24h" });
